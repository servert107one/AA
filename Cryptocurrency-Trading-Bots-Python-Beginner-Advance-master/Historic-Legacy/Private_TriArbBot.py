...
def binance_portfolio(coin_list):
    binance_portfolio = []
    for curr in coin_list:
        balance = client.synced('get_asset_balance', asset=curr)
        binance_portfolio.append(balance)
    data_log_to_file(binance_portfolio)
    portf_file_save(binance_portfolio, 'RealBinancePortfolio.txt')

def data_log_to_file(message):
    with open('CryptoTriArbBot_DataLog.txt', 'a+') as f:
        f.write(message)

def portf_file_save(portfolio, portf_file='Portfolio.txt' ):
    with open(portf_file, 'a+') as f:
        f.write('\n'+str(portfolio))

def arbitrage_bin(list_of_sym, tickers, portfolio, cycle_num=10, cycle_time=30, place_order='No', real_order='No'):
                                ...
                                for a, sym in enumerate(list_of_sym):
                                    ...
                                    if i==0:
                                        order = client.synced('create_order',
                                                    ...
                                                    timeInForce=15,
                                                    quantity=quantity[int(a)],
                                                    price=float(exch_rate_list[int(a)]))
                                        i+=1
                                    else:
                                        order = client.synced('create_order',
                                                    symbol=sym,
