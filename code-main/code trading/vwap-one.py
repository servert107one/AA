...
def main(show_plot=False):
    ...
    # Find the signal
    CLOSE_DISTANCE = 100
    ohlc["diff_vwap_high"] = (ohlc["vwap"] - ohlc["High"]).abs()
    ohlc["diff_vwap_low"]  = (ohlc["vwap"] - ohlc["Low"]).abs()
    ohlc["min"]            = ohlc[["diff_vwap_high", "diff_vwap_low"]].min(axis=1)
    first_condition        = (ohlc["ema_signal"] == 1) & (ohlc["vwap_signal"] == 1) & (ohlc["min"] <= CLOSE_DISTANCE)    
    second_condition       = (ohlc["ema_signal"] == -1) & (ohlc["vwap_signal"] == 2) & (ohlc["min"] <= CLOSE_DISTANCE)
