...
class EMAScaplingStrategy(Strategy):
    initsize = 0.4
    mysize   = initsize
...
def plot_ohlc_plus_signal(ohlc: pd.DataFrame) -> None:
    ...
    fig = go.Figure(data=[go.Candlestick(x=ohlc_subset.index,
                    ...
                    go.Scatter(x=ohlc_subset.index, y=ohlc_subset.ema50, line=dict(color='orange', width=1), name="EMA 50"),
                    go.Scatter(x=ohlc_subset.index, y=ohlc_subset.ema100, line=dict(color='blue', width=1), name="EMA 100"),
                    go.Scatter(x=ohlc_subset.index, y=ohlc_subset.ema150, line=dict(color='red', width=1), name="EMA 150")])
...
def generate_signal(ohlc: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in range(0, len(ohlc)):
        signal[row] = 0
        if ohlc.ema_signal[row]==1 and ohlc.Open[row]>ohlc.ema50[row] and ohlc.Close[row]<ohlc.ema50[row] and ohlc.High[row]-ohlc.Open[row]<=wick_limit:
            signal[row]=1

        if ohlc.ema_signal[row]==2 and ohlc.Open[row]<ohlc.ema50[row] and ohlc.Close[row]>ohlc.ema50[row] and ohlc.Open[row]-ohlc.Low[row]<=wick_limit:
            signal[row]=2   
...
def main(animation: bool = False, static_plot: bool = False, signal_plot: bool = True, backtest: bool = True):
    """
    ...
    # Run backtest
    if backtest:
        bt   = Backtest(ohlc, EMAScaplingStrategy, cash=100, margin=1/100, commission=.00)
        stats = bt.run()
...
if __name__ == "__main__":
    main(signal_plot = False, backtest = True)